...
- **[AgriciDaniel/plan-gate](https://github.com/AgriciDaniel/plan-gate)** (MIT) — **gate depth** (none / one-line / full read-only plan-first) and the calibration behind it: their A/B evidence that a full gate buys nothing on well-specified tasks, the Codex fresh-pass finding (a session cannot pause, gain permissions, and resume — the plan carries context), the "restrict the capability, don't ask nicely" enforcement principle (AP15), and the empty-plan-is-a-failure rule. Cited in-repo as PG1–PG3.
